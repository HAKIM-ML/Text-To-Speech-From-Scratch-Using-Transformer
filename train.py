...
def inference_utterance(model, text):
    ...
    ax1.imshow(postnet_mel[0, :, :].detach().cpu().numpy().T)
...
if __name__ == "__main__":
    ...
    if os.path.isfile(train_saved_path):  
        ...
        print(f"Load: {i}; epoch: {epoch}; test_loss: {np.round(best_test_loss_mean, 5)}; train_loss: {np.round(best_train_loss_mean, 5)}")
    else:
